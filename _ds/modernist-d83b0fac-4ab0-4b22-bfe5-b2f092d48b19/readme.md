...
Interactive states are themed, never browser defaults: give every interactive element a `:hover` tint and a pressed state from the accent ramp (one step past the base — `--color-accent-600` on a light ground, `--color-accent-400` on a dark one, or a `color-mix()` tint for outlined/ghost variants), and style keyboard focus with `:focus-visible { outline: 2px solid var(--color-accent); outline-offset: 2px; }` — never leave the browser's default focus ring.

The accent base sits between ramp steps 600 and 700, so on this light ground "one step past the base" means `--color-accent-700` for hover and `--color-accent-800` for pressed; step 600 is *lighter* than the base and inverts the feedback.
...
States are built in: hovers and pressed states come from the accent ramp, keyboard focus is the 2px accent `:focus-visible` ring, `::selection` is an accent tint, and disabled controls drop to 45% opacity. Don't restyle them per page. The accent-to-ground pair measures 5.78:1 — enough for body copy as well as icons, large text and interface chrome — and `--color-bg` type on an accent field measures the same, so the poster statement carries paragraph text safely. `--color-accent-700` (6.21:1) remains the choice where accent text sits alongside ink and needs a touch more weight.
...
- Use the accent sparingly, for the primary action and small emphasis; the system is mostly ink on ground. The one place the accent runs as a field is the poster statement — the deck's section dividers and the landing's closing banner — where type stays display-grade and the accent carries the page.
